...
public abstract class ValueAxis<T extends Number> extends Axis<T> {
    ...
    @Override public void invalidateRange(List<T> data) {
        ...
        for(T dataValue: data) {
            // skip NaN! ZCH: 22Aug2014
            if (Double.isNaN(dataValue.doubleValue()) != true) {
    ...
    @Override public double getDisplayPosition(T value) {
        double v = 0;
        if (Double.isNaN(value.doubleValue()) == true || Float.isNaN(value.floatValue()) == true)
            v = Double.NaN;
        else
            v = Math.round(offset + ((value.doubleValue() - currentLowerBound.get()) * getScale()));
        
        return v;
        //ZCH:[Handled Float.NaN]
